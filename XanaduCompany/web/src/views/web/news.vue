          ...
          <!-- <el-card shadow = "never" v-for = "(item,index) in recomNews" :key = "index">
            <router-link :to = "`/news/${item.id}`">
              <div class = "news-card-item">
                <img :src = "item.id" alt = "">
                <p class = "item-mask"><span>{{ item }}</span></p>
              </div>
            </router-link>
          </el-card> -->
        </div>
...
export default {
  ...
  methods: {
    handleSizeChange(val) {
      ...
      if (this.checkName === "1") {
        this.newsItems = {};
        this.getGroupDynasty();
      } else if (this.checkName === "2") {
        this.newsItems = {};
        this.getOther();
      } else if (this.checkName === "3") {
        //name ==3
        this.newsItems = {};
        this.studentDynasty();
      }
      console.log(`每页 ${this.pageInfo.count} 条`);
    },
    ...
    async getGroupDynasty() {
      const { data: res } = await this.$http.get("/groupDynamic/info", {
      ...
      if (res.meta.code !== 200) {
        ...
        if (this.newsItems.count <= this.newsItems.count) {
    ...
    async getRecomNews() {
      const { data: res } = await this.$http.get("/news/info", {
        params: this.pageInfo,
      });
      this.total = res.data.count;
      if (res.meta.code !== 200) {
    ...
    async searchByDate(data) {
      let t = data.split("-");
      let timerData = t[0] + "年" + t[1] + "月";
      let path = "/search/date?timerData=";
      if (this.checkName === "1") {
        path = "/groupDynamic" + path;
      } else if (this.checkName === "2") {
        path = "/notices" + path;
      } else {
        path = "/news" + path;
      }
      this.SearchByDate(path,timerData)
    },
    async SearchByDate(path,timerData) {
      const { data: res } = await this.$http.get(path + timerData);
      if (res.meta.code !== 200) {
        this.newsItems = {};
      } else {
